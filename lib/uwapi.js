...
export default class api {
	...
	constructor(apiToken) {
		...
		return this.compileAPISpec(spec);
	}

	//Should probably move this into a seperate module
	getJSON(url, data, cb, cberr) {
			...
			var endpointFunction = (function(templates, fname, cached) {
				return function(options, getParams) {
					...
					for (var i in templates) {
						path = this.constructEndpoint(templates[i], options);
						if(path)
							break;
					}
					...
					this.getJSON(url, getParams, function(payload){
